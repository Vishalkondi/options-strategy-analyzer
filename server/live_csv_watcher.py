...
class LiveCsvWatcher:
    ...
    def start(self) -> None:
        ...
        logger.info("Live CSV watcher started: %s", settings.LIVE_DIR)
    ...
    async def _process_file(self, path: Path) -> None:
        try:
            ...
            for row in rows:
                try:
                    await live_service.ingest(row, source="csv")
                except Exception as exc:
                    self.invalid_rows += 1
                    self.last_error = f"{path.name}: {exc}"
                    logger.warning("Skipping invalid row in %s: %s", path.name, exc)
            if rows:
    ...
    def _read_new_rows(self, path: Path) -> list[dict[str, str | None]]:
        ...
        previous_count = self._processed_rows.get(path, 0)
